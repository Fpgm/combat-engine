...
export const MobaDraft: React.FC<MobaDraftProps> = ({
  blueTeamName,
  redTeamName,
  opponentPool = [],
  teamSize,
  ...
  const [selectedBanChamp, setSelectedBanChamp] = useState<Champion | null>(null);
  const [selectedPickChamp, setSelectedPickChamp] = useState<Champion | null>(null);
  const [aiThinking, setAiThinking] = useState(false);

  // Build available champions (not banned, not picked)
  ...
  // AI ban selection — picks random from available
  ...
  // AI pick selection — prefers opponent pool, then random
  ...
  const handleBanChampion = (champ: Champion) => {
    ...
    const totalBansNeeded = teamSize * 2; // 10 bans for 5v5

    if (totalBansSoFar >= totalBansNeeded) {
      // All bans done, transition to pick
      setDraftPhase('pick');
      setBanTeam(null);
      setPickTeam('blue');
    } else {
  ...
  const isAvailable = (champId: string) => {
    return !bannedIds.includes(champId) && !pickedChampionIds.includes(champId);
  };

  const getStatusText = () => {
    if (draftPhase === 'ban') {
      const bansLeft = teamSize * 2 - (blueBans.length + redBans.length);
      return `BAN — ${banTeam?.toUpperCase() || 'BLUE'} bans next (${bansLeft} left)`;
    }
    if (draftPhase === 'pick') {
      return `PICK — ${pickTeam?.toUpperCase() || 'BLUE'} picks (${bluePicks.length}/${teamSize} B, ${redPicks.length}/${teamSize} R)`;
    }
    return `TACTICS — Confirm formations`;
  };

  const totalBans = teamSize * 2;
  ...
  return (
    <div className="flex flex-col items-center w-full p-4">
      ...
      <div className="flex items-center justify-between w-full max-w-4xl mb-4">
        ...
        <h2 className="font-black text-2xl font-mono text-white">DRAFT — {teamSize}v{teamSize} MOBA</h2>
        <div className="flex items-center gap-2">
          <span className="font-mono text-xs text-amber-400">{getStatusText()}</span>
        </div>
      </div>
      ...
      {draftPhase === 'tactics' && (
        <div className="w-full max-w-4xl bg-slate-900 border border-cyan-500/30 rounded-xl p-6">
          ...
          <button
            ...
            className="w-full py-3 bg-gradient-to-r from-cyan-500 to-indigo-600 hover:from-cyan-400 hover:to-indigo-500 text-slate-950 font-black font-mono rounded-xl"
          >
            CONFIRM FORMATION & ENTER COMBAT
          </button>
        </div>
      )}
    </div>
