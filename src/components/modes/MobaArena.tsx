...
export const MobaArena: React.FC<MobaArenaProps> = ({ initialEntities, blueTeamName, redTeamName, onMatchComplete }) => {
  ...
  useEffect(() => {
    if (phase === 'prepare') {
      const timer = setTimeout(() => setPhase('combat'), 3000);
  ...
  useEffect(() => {
    ...
    animFrameRef.current = requestAnimationFrame(() => {
      ...
      if (blueAlive.length === 0 || redAlive.length === 0) {
        ...
        const blueKills = result.entities ? result.entities.filter((e: CombatEntity) => e.team === 'blue' && e.isDead && e.killCount > 0).length : 0;
        const redKills = result.entities ? result.entities.filter((e: CombatEntity) => e.team === 'red' && e.isDead && e.killCount > 0).length : 0;
        onMatchComplete({ winner, stats: { blueKills, redKills, blueDamage: 0, redDamage: 0 } });
        return;
      }
    });
  ...
  return (
    <div className="flex flex-col items-center w-full">
      <div className="flex items-center justify-between w-full max-w-4xl mb-2">
        ...
        <span className="font-mono text-xs text-slate-400">PHASE: {phase.toUpperCase()}</span>
        <div className="flex items-center gap-2">
      ...
      <canvas
        ...
        width={800}
        height={500}
        className="border-2 border-slate-700 rounded-xl cursor-crosshair bg-slate-900"
      ...
      <div className="flex items-center gap-4 mt-2">
        ...
        <span className="font-mono text-xs text-slate-500">SPACE = PAUSE | 1 = ULTIMATE</span>
      </div>
    </div>
  );
};
